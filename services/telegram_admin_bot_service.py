            ...
            if query:
                ...
                if action == "confirm_user":
                    ...
                    keyboard = [
                        ...
                        [
                            InlineKeyboardButton(
                                ServicePackage.PRO.value, callback_data="PRO"
                            )
                        ],
                        [
                            InlineKeyboardButton(
        ...
        try:
            if query:
                ...
                if selected_package in ["BASIC", "PRO", "UNLIMITED"]:
                    ...
                    user = await self.user_service.update_user_package(
                        user_identifier, ServicePackage(selected_package)
                    )
                    if not user:
                    ...
                    if selection_type == "chat_id":
                        ...
                        if chat_id:
                            if (
                                ...
                                == ServicePackage.UNLIMITED
                            ):
                                # When upgrading to unlimited, automatically enable shift
                                await self.chat_service.update_chat_enable_shift(
                                    chat_id, True
                                )
                            else:
